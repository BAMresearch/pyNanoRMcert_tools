...
def f_lognormal_sphere(q, N, Rm, si, method=1):
    ...
    *si*: width parameter of size distribution
    ...
    if method == 1:
        ...
        def curve(q,N,Rm,si):
            x0=5*Rm  # integration up to 5 times the mean radius Rm
            # x0 is the independent variable here
            res = integrate.quad(integrand, 0, x0, args=(N,Rm,si,q))
            return res[0]
    ...
    if method == 2:
        def f(R):
            return (
                N
                / (np.sqrt(2 * np.pi) * si * R)
                * np.exp(-((np.log(R / Rm)) ** 2) / (2 * si * si))
                * (4.0 / 3.0 * np.pi * R**3) ** 2
                * (
                    3.0
                    * (sin(q * R) - q * R * cos(q * R))
